...
def load_data(num):
    src_text = read_file(f'./dataset/R8/原始文本-num{num}-seed2022.txt')
    steg_text_1 = read_file(f'./dataset/R8/合适度策略的隐写文本-num{num}-seed2022.txt')
    steg_text_2 = read_file(f'./dataset/R8/随机策略的隐写文本-num{num}-seed2022.txt')
    data = pd.DataFrame()
    data['text-1'] = src_text + steg_text_1
    data['text-2'] = src_text + steg_text_2
    data['label'] = [0] * num + [1] * num
    data = data.sample(frac=1, random_state=2022).reset_index(drop=True)
    n = int(num*0.9)
    dt1 = data[data.label==1].reset_index(drop=True)
    dt0 = data[data.label==0].reset_index(drop=True)
    
    train = pd.concat([dt0.iloc[:n],dt1.iloc[:n]],axis=0).sample(frac=1, random_state=2022).reset_index(drop=True)
    test = pd.concat([dt0.iloc[n:],dt1.iloc[n:]],axis=0).sample(frac=1, random_state=2022).reset_index(drop=True)
    data = pd.concat([train,test],axis=0).reset_index(drop=True)
    data['target'] = ['train']*len(train) + ['test']*len(test)
    
    return data

def read_train_test(exp_dataset : str, data_path:str = 'dataset', dataset2folder:dict = {'R8':'R8'}):
    print("\nLoading dataset..")
    dataset = []
    # R20
    if exp_dataset=='R8':
        ...
        for target in targets:
            ...
            with open(text_data_path,mode='r',encoding='utf-8') as f:
                lines = f.readlines()
            for line in lines:
                # print(line.strip())
                label, flag, text = line.strip().split('\t')

                # add doc
                dataset.append((target[:-4], label, text, flag))
    print("\tDataset Loaded! Total:", len(dataset))
    x = pd.DataFrame()
    x.to_csv('./results.csv',index=False)
    return pd.DataFrame(dataset, columns=["target", "label", "text",'flag'])

def read_data(exp_dataset : str,
              arg: list,
    ...
    if exp_dataset=='R8' or exp_dataset =='R52':
        targets = [f'train-random-{arg[1]}.txt', f'train-vector-{arg[1]}.txt',f'test-random-{arg[1]}.txt',f'test-vector-{arg[1]}.txt']
        for target in targets:
            text_data_path = os.path.join(data_path, dataset2folder[exp_dataset], target)
            with open(text_data_path,encoding='ISO-8859-1') as f:
                lines = f.readlines()
            for line in lines:
                # print(line.strip())
                ...
                # add doc
                dataset.append((target.split('-')[0], label, text))
